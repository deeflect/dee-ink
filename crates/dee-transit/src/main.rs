...
#[command(
    name = "dee-transit",
    version,
    about = "Find directions between two places",
    long_about = "dee-transit - Query directions APIs with agent-friendly JSON output.",
...
struct RouteArgs {
    ...
    #[arg(long, value_enum, default_value_t = ModeArg::Driving)]
    mode: ModeArg,
...
fn dispatch(cli: &Cli) -> AppResult<()> {
    match &cli.command {
        Commands::Route(args) => cmd_route(args, &cli.global),
        Commands::Config(args) => cmd_config(args, &cli.global),
    }
}

fn cmd_route(args: &RouteArgs, global: &GlobalArgs) -> AppResult<()> {
    validate_text(&args.origin, "origin")?;
    validate_text(&args.destination, "destination")?;

    let config = load_config_or_default()?;
    let api_key = config.api_key.ok_or(AppError::AuthMissing)?;
    let base_url = config
        .base_url
        .unwrap_or_else(|| "https://maps.googleapis.com/maps/api/directions/json".to_string());

    let url = format!(
        "{}?origin={}&destination={}&mode={}&alternatives={}&key={}",
        base_url,
        urlencoding::encode(args.origin.trim()),
        urlencoding::encode(args.destination.trim()),
        args.mode.as_api_value(),
        if args.alternatives { "true" } else { "false" },
    ...
    let parsed: DirectionsResponse =
        serde_json::from_str(&body).map_err(|_| AppError::ParseFailed)?;
    ...
    if items.is_empty() {
        ...
        for item in &items {
            println!(
                "{} | {}m | {}s",
                item.summary, item.distance_meters, item.duration_seconds
            );
        }
    }
...
fn map_route(route: RouteNode) -> Option<RouteItem> {
    let first_leg = route.legs.into_iter().next()?;
    Some(RouteItem {
        summary: route.summary,
        distance_meters: first_leg.distance.value,
...
fn validate_text(text: &str, field: &str) -> AppResult<()> {
...
fn config_path() -> AppResult<PathBuf> {
    ...
    Ok(base.join("dee-transit").join("config.toml"))
}
